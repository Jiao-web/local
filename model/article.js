...
class Article {
  ...
  static emotion(id, cb) {
    let sql = `SELECT emotion as x, count(*) as y FROM eyny_comment_tbl WHERE id=${id} group by emotion`;
    pool.getConnection(function(err, connection){
      if (err) return cb(err);
      connection.query(sql, cb);
      connection.release();
  ...
  static page(filter, cb) {
    ...
    try {
      if (article_id) {
        filter_str += ` id=${article_id} and`;
      }
  
      if (author_name) {
        filter_str += ` author_name='${author_name}' and`
      }
  
      if (date_start && date_end) {
        filter_str += ` created_at>'${date_start}' and created_at<'${date_end} ' and`;
      }
  
      if (filter_str) filter_str = 'where' + filter_str + ' 1';
